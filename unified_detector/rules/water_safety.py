...
- 支持容忍时间和首次报警时间
- 支持冷却时间
- 可见光：容忍时间内检测到3帧person就报警，不使用静止和平行过滤
- 热成像：使用静止和平行过滤
"""
import logging
import numpy as np
...
class WaterSafetyRule(RuleEngine):
    ...
    def _init_rule_specific(self):
        ...
        self.tolerance_time = 5.0  # 容忍时间，默认5秒
        ...
        # 获取相机类型：'0' 可见光；'1' 热成像
        self.user_type = self.device_info.get('user_type', '0')
        self.is_thermal = (self.user_type == '1')
        ...
        if self.is_thermal:
            ...
            # 可见光不使用过滤
            self.enable_static_filter = False
            self.enable_parallel_filter = False

        # 检测历史管理（滑动窗口）
        self.detection_history = []  # [(timestamp, bboxes), ...] person检测历史

        logger.debug(f"[{self.camera_key}] 涉水安全规则初始化: ROI数={len(self.rois)}, "
                    f"相机类型={'热成像' if self.is_thermal else '可见光'} (user_type={self.user_type}), "
    ...
    def process(self, frame, detections: List[Dict], timestamp: float) -> Optional[Dict]:
        ...
        if len(intruder_bboxes) > 0:
            if self.is_thermal:
                ...
                # 可见光：不过滤
                filtered_bboxes = intruder_bboxes
