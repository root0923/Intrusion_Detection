...
# 日志文件路径（按日期分割）
log_file = log_dir / f"water_safety_{datetime.now().strftime('%Y%m%d')}.log"
            ...
            for device in devices:
                ...
                for channel in channels:
                    ...
                    for rule in algorithm_rules:
                        if rule.get('algorithmCode') != 'water_safety':  # ！！！！涉水检测算法代码
                            continue
    ...
    def update_intrusion(self, detections: List[Dict], frame: np.ndarray) -> List[Dict]:
        ...
        if valid_detections:
            ...
            if self.intrusion_state['first_time'] is not None:
                duration = current_time - self.intrusion_state['first_time']
                logger.debug(f"涉水行为结束 (持续 {duration:.1f}s)")
                    ...
                    if alarms:
                        ...
                        for alarm in alarms:
                            alarm_data = {
                                ...
                                "alarmType": "water_safety",  # ！！！！涉水检测报警类型代码
                                "alarmTypeName": "涉水检测"
                            }
...
def _visualize_detections(frame: np.ndarray, roi_manager: ROIManager,
    ...
    # 显示报警信息
    if alarms:
        alarm_text = "ALARM! WATER SAFETY DETECTED"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='涉水安全检测系统')
